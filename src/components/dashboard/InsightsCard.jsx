  ...
  return (
    <Card className="p-5 h-full">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="um-label">{t("dash.insights")}</h2>
      </div>
      {insights.length === 0 ? (
        <div>
          <p className="text-sm text-muted-foreground">{t("dash.insights.empty")}</p>
          <Button size="sm" variant="outline" className="mt-4" onClick={() => navigate("/focus")}>
            <Play className="w-3.5 h-3.5 mr-1.5" />{t("dash.insights.start")}
          </Button>
        </div>
      ) : (
      ...
      <Link to="/insights" className="inline-flex items-center text-xs text-muted-foreground hover:text-foreground transition-colors mt-4">
        {t("dash.allInsights")} <ArrowRight className="w-3 h-3 ml-1" />
      </Link>
    </Card>
  );
}